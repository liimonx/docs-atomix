...
export const ComponentShowcase: React.FC<ComponentShowcaseProps> = ({ showcases }) => {
  if (!showcases || showcases.length === 0) {
    return (
      <Card>
        <div className="card-body">
          <p className="u-text-muted u-mb-0">No showcases available for this component.</p>
        </div>
      </Card>
    );
  }
    ...
    <Card className="u-mb-6">
      ...
      <div className="u-d-flex u-flex-column u-gap-4">
        {showcases.map((showcase, index) => (
          <div key={index} className="u-d-flex u-flex-column u-gap-2">
            ...
            <div className="u-d-flex u-flex-wrap u-gap-2">
              {showcase.tags.map((tag, tagIndex) => (
                <Badge key={tagIndex} label={tag} variant="secondary" size="sm">{tag}</Badge>
              ))}
            </div>
