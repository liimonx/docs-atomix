"use client";

import { FC } from "react";
import MarkdownPage from "@/components/documentation/MarkdownPage";

const CLIMigrationGuidePage: FC = () => {
  return (
    <MarkdownPage
      title="Atomix CLI - Migration Guide"
      description="Migrate from other CSS frameworks to Atomix Design System"
      markdownPath="/atomix-doc-in-md/docs/CLI_MIGRATION_GUIDE.md"
      heroImageSrc="https://images.unsplash.com/photo-1555066931-4365d14bab8c?ixlib=rb-4.1.0&auto=format&fit=crop&q=80&w=2728"
    />
  );
};

CLIMigrationGuidePage.displayName = "CLIMigrationGuidePage";

export default CLIMigrationGuidePage;







